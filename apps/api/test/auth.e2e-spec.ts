...
describe('POST /auth/refresh', () => {
  ...
  it.each([{}, { refresh_token: 42 }, { refresh_token: '' }])(
    ...
    async (payload) => {
      const response = await request(app.server).post('/auth/refresh').send(payload).expect(422)
...
describe('protected routes', () => {
  ...
  it('refuses a request with a malformed token', async () => {
    await request(app.server).get('/requests').set('Authorization', 'Bearer not.a.jwt').expect(401)
  })
})
...
describe('authenticated access to a running route', () => {
  it('lets a FINANCE user through', async () => {
    ...
    await request(app.server).get('/requests').set('Authorization', `Bearer ${token}`).expect(200)
  })

  it('lets a REQUESTER through, scoped to their own requests', async () => {
    const token = await app.tokenFor('solicitante@gex.test', 'GexRequester123!')
  ...
  it('refuses a request signed with the wrong secret', async () => {
    ...
    await request(app.server).get('/requests').set('Authorization', `Bearer ${forged}`).expect(401)
  })
})

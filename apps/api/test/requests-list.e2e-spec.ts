...
const list = (token: string, query = '') =>
  request(app.server).get(`/requests${query}`).set('Authorization', `Bearer ${token}`)
...
describe('GET /requests pagination', () => {
  ...
  it.each(['?page=0', '?page=-1', '?page=abc', '?page_size=0', '?page_size=-5'])(
    'rejects %s with 422 instead of a database error',
    async (query) => {
...
describe('GET /requests filters', () => {
  ...
  it('filters by due-date range inclusively on both ends', async () => {
    const response = await list(finance, '?due_from=2026-09-10&due_to=2026-09-10').expect(200)
    const body = response.body as RequestListResponse
  ...
  it('rejects an inverted due-date range with a clear message', async () => {
    const response = await list(finance, '?due_from=2026-09-30&due_to=2026-09-01').expect(422)
    const body = response.body as ErrorEnvelope
  ...
  it('marks exactly the overdue rows against APP_TODAY, not the wall clock', async () => {
    ...
    const expectedOverdueInvoiceNumbers = readSeedRequests()
      .filter(
        (row) =>
          (row.status === 'PENDING' || row.status === 'APPROVED') && row.due_date < '2026-09-18',
      )
  ...
  it('does not mark a request due exactly on APP_TODAY as overdue', async () => {
    const response = await list(finance, '?due_from=2026-09-18&due_to=2026-09-18').expect(200)
    const body = response.body as RequestListResponse

...
const summary = (token: string) =>
  request(app.server).get('/dashboard/summary').set('Authorization', `Bearer ${token}`)
...
describe('GET /dashboard/summary for finance', () => {
  ...
  it('excludes a payment made in the previous month', async () => {
    ...
    expect(body.paid_this_month_amount_cents).toBe(expected.finance.paid_this_month_amount_cents)
  })
})
...
describe('GET /dashboard/summary for each requester', () => {
  ...
  it('never lets a requester see the global totals', async () => {
    ...
    expect(body.pending_amount_cents).not.toBe(expected.finance.pending_amount_cents)
  })
})
...
describe('cache isolation across viewers', () => {
  it('gives each of the three viewers their own numbers, never one another’s', async () => {
    ...
    expect(financeBody.pending_amount_cents).toBe(expected.finance.pending_amount_cents)
    expect(anaBody.pending_amount_cents).toBe(expected.requesters[ANA_ID].pending_amount_cents)
    expect(brunoBody.pending_amount_cents).toBe(expected.requesters[BRUNO_ID].pending_amount_cents)
...
describe('cache behaviour', () => {
  it('reflects a new request immediately after it is created', async () => {
    ...
    expect(after.pending_amount_cents).toBe(before.pending_amount_cents + 10_000)
  })
  ...
  it('also invalidates what finance sees when a requester creates a request', async () => {
    ...
    expect(after.pending_amount_cents).toBe(before.pending_amount_cents + 20_000)
  })
  ...
  it('reflects a decision (approve) immediately for both the requester and finance', async () => {
    ...
    expect(afterAna.pending_amount_cents).toBe(beforeAna.pending_amount_cents - 9999)
    expect(afterAna.approved_amount_cents).toBe(beforeAna.approved_amount_cents + 9999)
    expect(afterFinance.pending_amount_cents).toBe(beforeFinance.pending_amount_cents - 9999)
    expect(afterFinance.approved_amount_cents).toBe(beforeFinance.approved_amount_cents + 9999)
  })
  ...
  it('reflects a mark-paid immediately for both the requester and finance', async () => {
    ...
    expect(afterAna.approved_amount_cents).toBe(beforeAna.approved_amount_cents - 150_000)
    expect(afterAna.paid_this_month_amount_cents).toBe(
      beforeAna.paid_this_month_amount_cents + 150_000,
    )
    expect(afterFinance.approved_amount_cents).toBe(beforeFinance.approved_amount_cents - 150_000)

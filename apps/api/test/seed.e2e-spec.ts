...
import { seed, type SeedAuditEvent, type SeedRequest, type SeedUser } from '../prisma/seed'
...
describe('seed', () => {
  it('loads every user, request and audit event', async () => {
    expect(await prisma.user.count()).toBe(read<SeedUser>('seed_users.json').length)
    expect(await prisma.request.count()).toBe(read<SeedRequest>('seed_requests.json').length)
  ...
  it('preserves changes made after the first run', async () => {
    ...
    expect((await prisma.user.findUniqueOrThrow({ where: { id: financeId } })).passwordHash).toBe(
      user.passwordHash,
    )
  })

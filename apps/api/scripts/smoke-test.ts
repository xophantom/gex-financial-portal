...
const DATABASE_URL =
  process.env.DATABASE_URL ?? 'postgresql://gex:gex_local_password@localhost:5432/gex_finance_test'
...
const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET ?? 'smoke-test-jwt-refresh-secret'
...
async function waitForBoot(lines: string[]): Promise<void> {
  ...
  fail(`the app never logged "Nest application successfully started" within ${BOOT_TIMEOUT_MS}ms`)
}
...
async function waitForCorrelatedLine(
  ...
  while (Date.now() - start < timeoutMs) {
    const found = parseLines(lines).find((line) => line.correlationId === CORRELATION_ID)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(`[smoke] GET /rota-inexistente with x-correlation-id: ${CORRELATION_ID}`)
    ...
    console.log('[smoke] OK: sanitised 404 envelope + correlated log line, both confirmed')
  } finally {
    child.kill('SIGTERM')
  }
}

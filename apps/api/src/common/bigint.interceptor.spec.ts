...
const run = async (payload: unknown) => {
  ...
  return new Promise((resolve, reject) => result.subscribe({ next: resolve, error: reject }))
}

describe('BigIntInterceptor', () => {
  it('converts a top-level BigInt to a number', async () => {
    await expect(run({ amount_cents: 125000n })).resolves.toEqual({ amount_cents: 125000 })
  })

  it('converts BigInt nested in arrays and objects', async () => {
    await expect(run({ data: [{ amount_cents: 1n }, { amount_cents: 2n }] })).resolves.toEqual({

...
function decimalToBigInt(value: Prisma.Decimal): bigint {
  if (!value.isInteger()) {
    throw new Error(`expected an integer amount in cents, got ${value.toString()}`)
  }
...
export class DashboardRepository {
  ...
  async summary(
    ...
    const scope =
      viewer.role === 'FINANCE' ? Prisma.sql`TRUE` : Prisma.sql`requester_id = ${viewer.id}::uuid`
    ...
    return {
      ...
      paid_this_month_amount_cents: decimalToBigInt(row.paid_this_month_amount_cents),
    }
  }
}
